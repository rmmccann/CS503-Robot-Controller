...
{ 
  ...
  while(1){
    updatePings();
    if(mmFrontCur < 150) digitalWrite(A5, HIGH);
    delay(500);
    digitalWrite(A5, LOW);
  }
  */
...
{
  mmFrontLast = mmFrontCur;
  //mmFrontLast2 = mmFrontCur2;
  
  mmSideLast = mmSideCur;
  //mmSideLast2 = mmSideCur2;
...
void turningLeft()
{
  ...
  /*
	if(mmFrontCur < 150)
   	...
   	if(mmFrontCur > 150+50 && mmSideCur < maxGoodRange) //TODO work on this condition (should wait a little more)
   	{
   		STATE = MOVING;
   	}
   */
...
void turningRight()
{
  //Serial.println("HERP DERP PRINT");
    ...
    while(!wallFound){
      ...
      //if it doesn't see a lost edge, then it's just a 90deg turn and MOVING can take over
        ...
        if(mmSideCur < 300){
          ...
          /*
          //Serial.println("DETECTED CLOSE THING");
          delay(1000);  //high enough that robot would clear 180deg turn, but short enough that it won't make wide turn on 90deg
          pingSide();
          if((mmSideCur - mmSideLast) > 100){
